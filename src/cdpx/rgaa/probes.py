...
(async () => {
  ...
  if (!candidates.length) return JSON.stringify({candidates: 0, examined, nodes_examined: examined, bytes_examined: bytesExamined, subtree_truncated: subtreeTruncated, execution_timed_out: false, clipped: [], clipped_total: 0, potential_overflow: [], potential_overflow_total: 0, truncated: true, evidence_complete: false, coverage_scope: "bounded light-DOM text nodes; readability, frames, shadow DOM and application side effects require review"});
