...
.PHONY: help setup check-local check lint fmt test test-e2e test-symfony-e2e test-shopware-e2e cov typecheck fixtures mock site-casts worktree-id docker-build docker-check docker-e2e docker-symfony-e2e docker-shopware-e2e proof release bump clean dist smoke-dist
