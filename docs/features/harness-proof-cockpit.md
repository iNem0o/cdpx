...
journey = "run-quality-gate"
title = "Isolate parallel Git worktrees"
ui_text = "Independent agents can build and validate from separate worktrees without sharing mutable Docker or proof resources."
...
[[scenarios]]
id = "resolve-stack-hosts-in-real-runtime"
journey = "ship-runtime"
title = "Resolve declared stack hosts inside a real runtime container"
ui_text = "A runtime joined to a development stack network resolves the hostnames declared in cdpx.yaml, driven by exported environment variables."
report_text = "This scenario proves on a real Docker daemon that an interpolated network name places the container on the stack network and that extra_hosts entries resolve inside it, with the full command transcript as capture."
