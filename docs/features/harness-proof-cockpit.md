...
report_text = "This scenario proves that the cockpit SPA renders each of its steering views in file:// alone: feature → journey → scenario → test card drill-down, the « À lire d'abord » panel and gaps on a red run, the run timeline and casts, the full CLI surface, the validation matrix and project context, with an explicit fallback on an unknown route."
