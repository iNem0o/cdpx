...
def test_render_html_size_stays_bounded():
    ...
    # pack, adversarial DOM/cleanup qualification, targeted scenario profiler
    # capture, bounded profiler probing and documented maintenance fixes. These
    # prevent broad test globs from being mistaken for runtime proof while
    # keeping the public change record inside the cockpit.
    #: beyond the ceiling, an asset grew without justification: the gate blocks it
    assert len(proof.render_html(summary)) < 4_610_000
